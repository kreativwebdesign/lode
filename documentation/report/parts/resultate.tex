...
Anschliessend werden die neuen Informationen in \e{Buffer} gespeichert. Als Beispiel hierzu ist die Generierung des \e{Vertexbuffers} in Abbildung \ref{code:bufferCreation} ersichtlich. Ein \e{Mesh} kann mehrere \e{Primitives} beinhalten. Ein \e{Primitive} definiert einen Teil des Modells, dies beinhaltet unter anderem \e{Vertices} und \e{Material}-Informationen. Die Variable \e{primitiveIndex} speichert den Index des aktuellen \e{Primitives}. In der Variable \e{verticesAsFloat} werden die neuen \e{Vertices} als \e{Float32Array} abgespeichert.
