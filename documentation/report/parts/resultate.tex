...
\section{\e{LOD}-Pipeline}

Der Kern der Arbeit stellt die \e{LOD}-Pipeline, \e{lode} genannt, dar. Dieses Tool liefert eine für das Web optimierte Möglichkeit, \e{LOD}-Artefakte für eine Vielzahl von Anwendungsgebieten zu generieren. Sämtliche Teile der Arbeit, insbesondere der Quellcode, stehen offen zur Verfügung und können online eingesehen werden \cite{lode}.
...
\begin{figure}[H]
  ...
  \caption{lode \e{CLI} config – Konfiguriert die gewünschten \e{LOD}-Artefakten}
...
\begin{figure}[H]
  ...
  \caption{lode \e{CLI} run – Generiert die gewünschten \e{LOD}-Artefakte}
...
\todo[inline]{Benchmark sections prevent single child section auswertung}
\section{Benchmark}
...
Auch der zweite Durchlauf auf einem Dell-Gerät mit integrierter GPU führt zu ähnlichen Daten. Auffallend in \ref{fig:benchmarkFpsChartWindows} ist der Unterschied der \e{FPS}. Die Unterschiede der Downloadzeit (siehe \ref{fig:benchmarkDownloadChartWindows}) sind vergleichbar mit denjenigen vom ersten Durchlauf.
