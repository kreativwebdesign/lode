...
\paragraph{Imposters}
Vergleichbar zu LOD-Artefakten sind sogenannte \e{Imposters}. Dabei wird ein \e{Quad} mit verschiedenen Texturen verwendet. Für jede mögliche Position des Modells wird eine Textur generiert. Das \e{Quad} wird in Richtung Kamera ausgerichtet und abhängig von der Rotation des Modells die geeignete Textur verwendet. Geometrisch kann ein Modell so auf ein absolutes Minimum reduziert werden.
\todo[inline]{@simon nachteile -> beleuchtung}
...
\todo[inline]{@simon clustering erläutern (bei DLOD?)}
...
\todo[inline]{@simon scene graph glossary (entry refactoren)}
