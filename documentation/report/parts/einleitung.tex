...
In diversen \fglspl{Rendering Engine}{Programm, das zuständig für die Darstellung von 3D-Grafiken ist} gibt es deshalb Möglichkeiten für das Verwenden von sogenannten Level Of Details (LOD) Artefakten.
...
Zudem ist WebGL eine junge Technologie und wurde erst 2011 veröffentlicht – verglichen mit dem initialen Veröffentlichungsdatum von \fgls{OpenGL}{Spezifikation einer Programmierschnittstelle zur Entwicklung von 2D- und 3D-Grafikanwendungen} welches im Jahre 1992 publiziert wurde \cite{webGl1Spec,openGlSpec}.
...
\paragraph{Three.js}
\todo[inline]{Semantic versioning fussnote kürzen durch semver.org link}
...
Die Hauptvorteile liegen in der weiten Verbreitung, dem grossen Fokus auf Performanz und einem soliden Set an Basisfeatures. Elemente wie eine Physik-Engine fehlen jedoch und müssen vom Entwickler integriert werden. Zudem wird \e{Three.js} nicht mit \fGls{Semantic Versioning}{Jede Version wird mit einer aus drei Zahlen bestehenden Versionsbezeichnung (z.B. v1.0.5) versehen. Wobei jede dieser drei Zahlen eine gewisse Gewichtung hat. So wird die vorderste Zahl als \e{Major} bezeichnet. Diese wird erhöht, wenn grössere Änderungen hinzukommen, die möglicherweise nicht rückwärtskompatibel sind. Die mittlere Zahl – \e{Minor} genannt – wird erhöht, wenn neue Funktionen hinzugefügt werden, welche rückwärtskompatibel sind. Die hinterste Zahl ist für \e{Patches}, welche Probleme beheben, ohne die Kompatibilität zu beeinträchtigen.} entwickelt, was bedeutet, dass jede neue Version potenziell nicht rückwärtskompatibel Änderungen beinhalten kann.
...
Ein weiterer Kandidat ist \e{Babylon.js}, eine offene Bibliothek, welche die Entwicklung von 3D-Applikationen vereinfacht. \e{Babylon.js} zeichnet sich vor allem durch ein breites \e{Featureset} aus \cite{babylonjsNpmPackage}. Babylon.js verfügt zum Beispiel über Erweiterungen für verschiedene Physik-Engines und erleichtert somit den Einsatz von anspruchsvolleren Features. Ausserdem setzt es auf \e{Semantic Versioning}.
\todo[inline]{bad parts of babylon?}

\paragraph{PlayCanvas}
\e{PlayCanvas} ist eine offene 3D-Engine, welche primär für das Web entwickelt wurde. Als Erweiterung wird zudem ein proprietärer Cloud Service angeboten \cite{playcanvasNpmPackage}. Die Community ist – im Vergleich zur direkten Konkurrenten, wie zum Beispiel \e{Three.js} – klein. PlayCanvas verfolgt jedoch einen anderen Ansatz und liefert zum Beispiel eine standardmässige Integration für eine Physik-Engine.
...
\todo[inline]{Three.js simplification modifier}
\todo[inline]{Fehlende Teile erwähnen}
