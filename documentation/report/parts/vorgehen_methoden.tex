...
Google Chrome basiert auf \gls{Chromium}, dieselbe Engine, welche auch Microsoft Edge oder Opera verwenden.
...
Da jedoch das Analysieren der \gls{GPU} Daten stark vom System abhängig ist und dafür zusätzliche Komplexität notwendig wäre, wird in diesem Benchmark die im Google Chrome integrierten Chrome DevTools eingesetzt.
Selenium bietet zurzeit eine suboptimale Integration für das \emph{Chrome DevTools Protocol}.
\gls{Puppeteer}, eine weitere Option für die Automation, ist eine Bibliothek, die eine vereinfachte Schnittstelle zu einer Headless Chrome Instanz bietet.
...
\begin{enumerate}
  \item Öffne Applikation in \emph{\gls{Headless Chrome}} Instanz.
  \item Warte bis Seite geladen ist.
  \item Starte \emph{\gls{Profiling}}.
  \item Warte $n$ Sekunden.
  \item Stoppe \emph{Profiling}.
  \item Werte Daten aus.
\end{enumerate}

Der Test erfasst kontinuierlich die \emph{\gls{FPS}}. Zudem wird aus dem \emph{Profiling} eine Kennzahl bzgl. der GPU Nutzung berechnet.
