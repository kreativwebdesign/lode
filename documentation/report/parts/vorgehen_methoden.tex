...
\fgls{Puppeteer}{NodeJS Library, die eine API anbietet zum steuern von Chromium oder Chrome über das Chrome DevTools Protocol}, eine weitere Option für die Automation, ist eine Bibliothek, die eine vereinfachte Schnittstelle zu einer Headless Chrome Instanz bietet.
...
Der Test erfasst kontinuierlich die \fgls{FPS}{Frames per Second. Bilder pro Sekunde. Gängige Art, die Performanz der Rendering Engine zu messen}. Zudem wird aus dem \emph{Profiling} eine Kennzahl bzgl. der GPU Nutzung berechnet.
...
Anschliessend wird die Varianz der FPS für die unoptimierten respektive optimierten Werte berechnet. Die Varianz dient als Kennzahl, um eine statistische Signifikanz nachweisen zu können. Abschliessend werden die Werte der beiden Versionen verglichen. Angestrebt werden sollten 60 FPS. Wenn die unoptimierte Variante diesen Wert erreicht oder gar übersteigt, ist eine Optimierung nicht notwendig. Erreicht die optimierte Variante diesen Wert nicht, ist die Szenerie nach wie vor zu komplex. Ziel der Arbeit ist es, einen Algorithmus zu entwickeln, der für Szenerien unter 60 FPS diese erhöht und im Idealfall auf ebendiesen Wert hebt.
...
Nebst dem \e{CLI} soll folglich auch ein Javascript-Modul bereitgestellt werden, welches das Laden von glTF Dateien übernimmt und basierend auf der erkannten Performanz das beste Level of Detail einspielen.