...
Google Chrome basiert auf \fgls{Chromium}{Beschreibung}, dieselbe Engine, welche auch Microsoft Edge oder Opera verwenden.
...
Da jedoch das Analysieren der \fgls{GPU}{Beschreibung} Daten stark vom System abhängig ist und dafür zusätzliche Komplexität notwendig wäre, wird in diesem Benchmark die im Google Chrome integrierten Chrome DevTools eingesetzt.
Selenium bietet zurzeit eine suboptimale Integration für das \emph{Chrome DevTools Protocol}{Beschreibung}.
\fgls{Puppeteer}{Beschreibung}, eine weitere Option für die Automation, ist eine Bibliothek, die eine vereinfachte Schnittstelle zu einer Headless Chrome Instanz bietet.
...
\begin{enumerate}
  \item Öffne Applikation in \fgls{Headless Chrome}{Beschreibung} Instanz.
  \item Warte bis Seite geladen ist.
  \item Starte \fgls{Profiling}{Beschreibung}.
  \item Warte $n$ Sekunden.
  \item Stoppe \emph{Profiling}{Beschreibung}.
  \item Werte Daten aus.
\end{enumerate}

Der Test erfasst kontinuierlich die \fgls{FPS}{Beschreibung}. Zudem wird aus dem \emph{Profiling} eine Kennzahl bzgl. der GPU Nutzung berechnet.
